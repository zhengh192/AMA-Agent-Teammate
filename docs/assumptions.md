...
## A-34 - Bounded autonomous analysis loop

- For the local POC, schema-validated model assessment runs before keyword routing. Deterministic routing remains the fallback and still enforces Jira, explicit analysis, Knowledge explanation, and other routing invariants.
- Diagnose, compare, and explore tasks may run up to three observable analysis iterations. Each iteration is: prepare a natural-language step, compile and validate read-only SQL, obtain the existing exact SQL approval, execute, observe bounded evidence, and either finish or prepare the next step. The loop never emits private chain-of-thought.
- A follow-up iteration revises the existing per-run plan record because the current SQLite schema intentionally stores one plan row per run. Prior query proposals, executions, datasets, evidence, and step metadata remain independently persisted and referenced in the final result.
- Active Skills selected by strong content relevance may extend the static intent defaults. Skill instructions guide method, checkpoints, and completion signals; they do not bypass metadata resolution, SQL validation, or evidence checks.
- Generated Python is still prohibited in the API process. It can run only when the Docker sandbox is explicitly enabled; the container has no network, a read-only filesystem and input mount, dropped capabilities, a non-root user, and CPU, memory, process, time, input, and output limits. With no configured sandbox, analysis falls back to SQL and the controlled library.
- Knowledge, Skill, and Memory learning candidates produced after an analysis are visible proposals only. They do not change active behavior until the existing governed lifecycle approves them.

...
## A-34 - Skill-aware POC planning and graceful result bounds

- Active analysis Skill instructions are retrieved by request relevance and passed into typed task understanding and non-SQL query planning together with their IDs and versions. A model-recommended Skill is accepted only when the local active registry contains it; the final execution plan and trace remain authoritative.
- Skill edits can change the investigation steps, required evidence, and structured query interpretation without adding a new keyword branch. Catalog validation, read-only SQL compilation, AST policy, and evidence labels still apply after model planning.
- The local POC source policy allows up to 2,000 rows and 4 MiB per query by default. A connector that reaches a row or byte boundary returns a visibly marked truncated detail dataset instead of failing the whole run; aggregate claims must come from a complete aggregate query rather than truncated detail.
- Case-journey distribution rows and bounded bot-response evidence rows share one approved read-only query but remain distinct record types. Response text is not repeated across aggregate rows or used as a chart dimension.

  ...
  function handleServerEvent(event: ServerEvent, buffer: { text: string; runId: string | null }) {
    ...
    if (event.event === "approval.decision") {
      ...
      if (textValue(event.data.decision) === "changes_requested") {
        ...
        setError(null);
        setClarification(
          "Tell me what should change in the metric meaning, date range, dimensions, or SQL. I will use your correction in a new plan.",
        );
      } else {
        setStatus("failed");
        setError("The SQL plan was rejected. Submit a revised analytical request when ready.");
      }
    }
  ...
  return (
    <div className="app-shell">
      ...
      <main className="workspace phase-two-workspace">
        ...
        {approval ? <section className="analysis-card approval-card" aria-label="SQL review"><div className="card-heading"><div><span className="eyebrow">Approval required</span><h2>Analysis plan and SQL review</h2></div><span className="policy-chip">{approval.plan.policy_version}</span></div><p>{approval.plan.goal}</p><dl className="plan-grid"><div><dt>Method</dt><dd>{approval.plan.analysis_type}</dd></div><div><dt>Metric</dt><dd>{approval.plan.metric}</dd></div><div><dt>Chart</dt><dd>{approval.plan.chart_type}</dd></div><div><dt>Success</dt><dd>{approval.plan.success_criteria}</dd></div></dl>{approval.plan.metadata_confidence === "working_assumption" ? <div className="assumption-panel"><strong>Working assumption · Inferred</strong><p>This draft definition comes from the project documents and can be corrected in conversation.</p><ul>{approval.plan.assumptions.map((item) => <li key={item}>{item}</li>)}</ul></div> : null}{approval.plan.metadata_confidence === "learned_definition" ? <div className="assumption-panel"><strong>Learned definition · Confirmed by you</strong><p>This versioned calculation was taught in a prior conversation and still requires exact SQL approval.</p><ul>{approval.plan.assumptions.map((item) => <li key={item}>{item}</li>)}</ul></div> : null}{approval.plan.queries.map((query) => <div className="sql-review" key={query.id}><div><strong>{query.source_id}</strong><span>{query.dialect} · max {query.max_rows} rows · {query.timeout_seconds}s</span></div><pre><code>{query.sql}</code></pre><small>Parameters: {JSON.stringify(query.parameters)}</small></div>)}{approval.plan.join_plan ? <details open><summary>Validated cross-source join</summary><pre>{JSON.stringify(approval.plan.join_plan, null, 2)}</pre></details> : null}<div className="approval-actions"><button className="approve" type="button" onClick={() => void decide("approved")}>Approve and execute</button><button type="button" onClick={() => void decide("changes_requested")}>Request changes</button><button className="reject" type="button" onClick={() => void decide("rejected")}>Reject</button></div><small className="hash">Exact payload: {approval.payload_hash.slice(0, 16)}…</small></section> : null}
